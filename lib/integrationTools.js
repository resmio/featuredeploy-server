...
const makeGithubFeatureDeployComments = ({
  ...
  getJWT().then((token) => {
    ...
    }).catch((e) => { console.log(e) }) // end getGithubToken
        ...
        tools.commentOnPullRequest(pull.comments_url, 'removing featuredeploy...', ghToken).then((newComment) => {
          tools.getPullComments(pull.comments_url, ghToken).then((comments) => {
            ...
            tools.deleteMultipleComments(allBotComments, ghToken).then(() => {
              tools.deleteLabelFromPullRequest(pull.issue_url, 'featuredeploy', ghToken).catch((e) => { console.log(e) })
...
const getPullRequest = ({pullUrl, fullName, branchName, ghToken}) => {
  if (pullUrl) {
    ...
    return Promise.reject('not enough information to get pull request...')
  }
}
...
const checkForFeatureDeployLabel = ({
  ...
  return new Promise((resolve, reject) => {
    getJWT().then((token) => {
      tools.getGithubToken(installationId, token).then((ghToken) => {
        tools.getPullLabels(labelsUrl, ghToken).then((labels) => {
          if (labels) {
            resolve(labels.map((l) => l.name).contains('featuredeploy'))
          } else {
